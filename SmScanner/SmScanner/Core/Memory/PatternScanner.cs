...
namespace SmScanner.Core.Memory
{
	public class PatternScanner
	{
		...
		public static IntPtr FindPattern(BytePattern pattern, RemoteProcess process, Smdkd.SmModule module)
		{
		...
		/// <returns>The address of the pattern or <see cref="IntPtr.Zero"/> if the pattern was not found.</returns>
		public static IntPtr FindPattern(BytePattern pattern, RemoteProcess process, Smdkd.SmSection section)
		{
			Contract.Requires(pattern != null);
			Contract.Requires(process != null);
		...
		public static BytePattern CreatePatternFromCode(RemoteProcess process, IntPtr start, int size)
		{
			...
			try
			{
				...
				Smdkd.DisassembleCode(eip, size, IntPtr.Zero, true, (ref Smdkd.InstructionData instruction) =>
				{
