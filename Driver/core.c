#include "stdafx.h"
 ...
 INT64 CoreNtOpenProcess(PNTOPENPROCESS_ARGS args) {
	 ...
	 if (!SafeCopy(&clientId, args->ClientId, sizeof(clientId))) {
		 return STATUS_ACCESS_VIOLATION;
	 }
 ...
 INT64 CoreNtTerminateProcess(PNTTERMINATEPROCESS_ARGS args) {
	 HANDLE handle = 0;
	 PEPROCESS process = 0;
	 NTSTATUS status = PsLookupProcessByProcessId(DecodeHandle(args->ProcessHandle), &process);

	 DbgPrintEx(0, 0, "CoreNtTerminateProcess\n");
	 ...
	 case TerminateEx:
		 ...
		 status = ZwTerminateProcess(handle, args->ExitStatus); // Call ZwTerminateProcess with NULL handle
		 NtClose(handle);
		 break;
 ...
 INT64 CoreNtWaitForSingleObject(PNTWAITFORSINGLEOBJECT_ARGS args) {
	 ...
	 if (NT_SUCCESS(status)) {
		 ...
		 if (!SafeCopy(args->Status, &status, sizeof(NTSTATUS))) {
			 status = STATUS_ACCESS_VIOLATION;
		 }
		 NtClose(handle);
	 }
...
#pragma region Sm Ex
 NTSTATUS CoreObOpenObjectByPointer(PObOpenObjectByPointer_ARGS args) {
	 HANDLE		id		=	args->Id;
	 HANDLE		handle  =	0;
	 PETHREAD	thread  =	0;
	 PEPROCESS	process =	0;
	 ...
	 if (NT_SUCCESS(status = PsLookupProcessByProcessId(id, &process))) {
		 status = ObOpenObjectByPointer(process, 0, 0, args->Access, *PsProcessType, KernelMode, &handle);
		 ObDereferenceObject(process);
	 }
	 else if (NT_SUCCESS(status = PsLookupThreadByThreadId(id, &thread))) {
		 status = ObOpenObjectByPointer(thread, 0, 0, args->Access, *PsThreadType, KernelMode, &handle);
		 ObDereferenceObject(thread);
	 }

	 if(NT_SUCCESS(status)) {
		 if (!SafeCopy(args->Handle, &handle, sizeof(handle))) {
			 status = STATUS_ACCESS_VIOLATION;
		 }
	 }
